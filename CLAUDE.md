...
### 必須：Planモードでは必ず情報の正確性を確認すること

設計・提案の内容が正しいかどうか、実装に入る前にユーザーへ確認を取ること。

**確認すべき事項（例）：**
- 変更対象のファイルパス・関数名・型名が実際のコードと一致しているか
- 提案する実装方針がプロジェクトの既存設計と矛盾していないか
- 追加・変更するデータ（ブランド情報・店舗情報など）の内容が正しいか
- 外部仕様（API レスポンス形式・環境変数名など）に誤りがないか

> Planモードで示した情報が正しいかどうか、必ずユーザーに確認してから実装フェーズへ進むこと。
